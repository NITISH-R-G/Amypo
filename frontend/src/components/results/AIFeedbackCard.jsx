...
export default function AIFeedbackCard({ feedback }) {
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-2xl p-6 text-white shadow-xl shadow-indigo-900/20 relative overflow-hidden">
      {/* Decorative background elements */}
      <div className="absolute top-0 right-0 -transtale-y-1/2 translate-x-1/3 w-64 h-64 bg-indigo-500 rounded-full mix-blend-screen opacity-20 filter blur-3xl"></div>
      <div className="absolute bottom-0 left-0 translate-y-1/3 -translate-x-1/3 w-48 h-48 bg-purple-500 rounded-full mix-blend-screen opacity-20 filter blur-3xl"></div>
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-4">
           ...
           <div>
             ...
             <p className="text-indigo-200 text-xs font-medium uppercase tracking-wider">Powered by Gemini</p>
           </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2">
              {feedback.suggestions.map((suggestion, idx) => (
                <li key={idx} className="flex gap-3 text-sm text-indigo-50 bg-indigo-500/10 p-2.5 rounded-lg border border-indigo-500/20">
