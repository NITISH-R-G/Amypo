...
function calculateScores(data) {
  ...
  const secretsDetected = data.secretlint && Array.isArray(data.secretlint) && data.secretlint.some(file => file.messages && file.messages.length > 0);
  if (secretsDetected) {
...
async function analyzeWithAI(reportsData, scores) {
  ...
  const prompt = `
    ...
    - Secrets detected: ${reportsData.secretlint && Array.isArray(reportsData.secretlint) && reportsData.secretlint.some(file => file.messages && file.messages.length > 0) ? 'Yes' : 'No'}
