...
function calculateScores(data) {
  ...
  if (data.eslint) {
    ...
    const warningCount = data.eslint.reduce((acc, file) => acc + file.warningCount, 0);
    qualityScore -= Math.min((errorCount * 2) + warningCount, 40);
  }
  ...
  if (data.secretlint && data.secretlint.some(file => file.messages.length > 0)) {
