...
function calculateScores(data) {
  ...
  const hasSecretsDetected = data.secretlint && data.secretlint.some(file => file.messages.length > 0);

  if (hasSecretsDetected) {
...
async function analyzeWithAI(reportsData, scores) {
  ...
  const prompt = `
    ...
    - Secrets detected: ${reportsData.secretlint && reportsData.secretlint.some(file => file.messages.length > 0) ? 'Yes' : 'No'}
...
async function main() {
  ...
  let failCI = false;
  const hasSecretsToFail = data.secretlint && data.secretlint.some(file => file.messages.length > 0);
  if (hasSecretsToFail) {
